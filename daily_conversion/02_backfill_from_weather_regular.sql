-- 2_backfill_from_weather_regular.sql
-- Calculates what it can from weather_regular and appends to weather_daily.
...
WITH base AS (
    SELECT
        ...
        wind_speed,
        humidity,
        NULLIF(uvb_radiation, -1)  AS uvb_radiation,
        NULLIF(uvb_radiation2, -1) AS uva_radiation
...
SELECT
    ...
    NULL::double precision AS precipitation_max_10min_intensity,
    ...
    AVG(uvb_radiation) AS uvb_radiation_mean,
    MAX(uvb_radiation) AS uvb_radiation_max,
    MIN(uvb_radiation) AS uvb_radiation_min,
    SUM(uvb_radiation) AS uvb_radiation_sum,
    COUNT(*) FILTER (WHERE uvb_radiation > 5) AS uvb_radiation_count_gt_5,
    AVG(uvb_radiation) FILTER (WHERE uvb_radiation > 5) AS uvb_radiation_gt_5_mean,
    COUNT(*) FILTER (WHERE uvb_radiation > 10) AS uvb_radiation_bright_count,

    AVG(uva_radiation) AS uva_radiation_mean,
    MAX(uva_radiation) AS uva_radiation_max,
    MIN(uva_radiation) AS uva_radiation_min,
    SUM(uva_radiation) AS uva_radiation_sum,
    COUNT(*) FILTER (WHERE uva_radiation > 5) AS uva_radiation_count_gt_5,
    AVG(uva_radiation) FILTER (WHERE uva_radiation > 5) AS uva_radiation_gt_5_mean,
    COUNT(*) FILTER (WHERE uva_radiation > 10) AS uva_radiation_bright_count,
